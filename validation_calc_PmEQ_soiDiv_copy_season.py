...
if __name__ == '__main__':
    ...
    for ts in ts_list:    
        ...
        for i in [band_no]:   
            ...
            out_file_neg = drght_dir + 'PmEQ_results/soineg_mean_seas_glm_probability_prob_band' + str(band_name[i]) + '_weeks' + str(ts) + '.nc'
            sel_prob_band_soineg.groupby('time.season').mean('time').to_netcdf(out_file_neg)
            ...
            da_prob_drought_break_soineg = (da_exceed_events_prob_band_soineg.groupby('time.season').sum('time')/da_total_events_prob_band_soineg.groupby('time.season').sum('time')).rename('hist_prob')

            # out_file_pos = drght_dir + 'PmEQ_results/soipos_hist_prob_drought_break_band' + str(band_name[i]) + '.nc'
            out_file_neg = drght_dir + 'PmEQ_results/soineg_hist_seas_prob_drought_break_band' + str(band_name[i]) + '_weeks' + str(ts) + '.nc'
