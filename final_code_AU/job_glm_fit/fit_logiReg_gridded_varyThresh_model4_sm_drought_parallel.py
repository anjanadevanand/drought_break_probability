...
def identify_start_year(glm_dir, start_lat, end_lat, start_lon, end_lon):
    start_year = 1911
...
if __name__ == '__main__':
    ...
    main_dir = '/g/data/w97/ad9701/p_prob_analysis/processed_data/awra/'
    ...
    start_yr = identify_start_year(glm_dir = drght_dir + varname + '_week' + str(iW) + '/', start_lat = start_lat, end_lat = end_lat, start_lon = start_lon, end_lon = end_lon)
    print('start_yr=')
    print(start_yr)
    ...
    if start_yr is not None:
        ...
        overall_time_slice = slice('1911-01-01', '2020-12-31')
...
#***************************
#         doy = da_thresh['time.dayofyear'].values[i_time]
#         if ((i_time>0) & (doy==1)) | (i_time == len(da_thresh.time)-1):
#             prev_year = da_thresh['time.year'].values[i_time-1]
#             ds_glm = xr.open_mfdataset(full_dir_path + 'GLM_results_*' + str(prev_year) + '*.nc', combine = "nested", concat_dim = "time")
...
#             # save ds_glm
#             out_file = drght_dir + '/' + varname + '_week' + str(iW) + '/' + 'GLM_results_' + sub_dir + '_' + str(prev_year) + '.nc'
#             print(out_file)
#             ds_glm.to_netcdf(out_file)
 
#             fileList = glob.glob(full_dir_path + 'GLM_results_*' + str(prev_year) + '*.nc')
#             print(fileList[0])
#             for f in fileList:
#                 os.remove(f)
