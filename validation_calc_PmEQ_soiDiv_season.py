...
if __name__ == '__main__':
    ...
    for ts in ts_list:    
        ...
        for i in [band_no]:   
            ...
            # out_file_pos = drght_dir + 'PmEQ_results/soipos_glm_probability_prob_band' + str(band_name[i]) + '_weeks' + str(ts) + '.nc'
            ...
            out_file_pos = drght_dir + 'PmEQ_results/soipos_mean_seas_glm_probability_prob_band' + str(band_name[i]) + '_weeks' + str(ts) + '.nc'
            sel_prob_band_soipos.groupby('time.season').mean('time').to_netcdf(out_file_pos)
            ...
            da_prob_drought_break_soipos = (da_exceed_events_prob_band_soipos.groupby('time.season').sum('time')/da_total_events_prob_band_soipos.groupby('time.season').sum('time')).rename('hist_prob')
            ...
            out_file_pos = drght_dir + 'PmEQ_results/soipos_hist_seas_prob_drought_break_band' + str(band_name[i]) + '_weeks' + str(ts) + '.nc'
