...
if __name__ == '__main__':
    ...
    drght_time_slice = slice('1961-01-01', '2020-05-31')
    ...
    for ts in ts_list:    
        ...
        for i in [band_no]:   
            ...
            # out_file = drght_dir + 'PmEQ_results/post1960_glm_probability_prob_band' + str(band_name[i]) + '_weeks' + str(ts) + '.nc'
            # sel_prob_band.to_netcdf(out_file)
            out_file = drght_dir + 'PmEQ_results/post1960_mean_seas_glm_probability_prob_band' + str(band_name[i]) + '_weeks' + str(ts) + '.nc'
            sel_prob_band.groupby('time.season').mean('time').to_netcdf(out_file)
            ...
            da_prob_drought_break = (da_exceed_events_prob_band.groupby('time.season').sum('time')/da_total_events_prob_band.groupby('time.season').sum('time')).rename('hist_prob')
            out_file = drght_dir + 'PmEQ_results/post1960_hist_seas_prob_drought_break_band' + str(band_name[i]) + '_weeks' + str(ts) + '.nc'
